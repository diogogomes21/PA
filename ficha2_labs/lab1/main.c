...
#define NUM_STRING 13
int main(int argc, char **argv)
{
	(void)argc;(void)argv; /* Silencia warnings */

	const char *vector_to_sort[NUM_STRING] = {
	...
	str_asort(vector_to_sort, NUM_STRING, sort_by_length);
	printf("Order by length:\n");
	print_vector(vector_to_sort, NUM_STRING);

	str_asort(vector_to_sort, NUM_STRING, sort_by_number_of_vogals);
	printf("Order by number of vogals:\n");
	print_vector(vector_to_sort, NUM_STRING);

	str_asort(vector_to_sort, NUM_STRING, strcmp);
