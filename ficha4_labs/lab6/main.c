...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < MAX_THREADS; i++){
		if ((errno = pthread_create(&tids[i], NULL, task,
						 &thread_params[i])) != 0)
...
void *task(void *arg)
{
	...
	if (params->id == 1){
		printf("Thread %d - sinalizará a variável de condição "
				" daqui a 5 segundos\n", params->id);
		sleep(5);
	}
	...
	/* A primeira thread (id==1) sinaliza uma variável de condição,
	 * as restantes esperam nessa variável */
	if (params->id == 1){
