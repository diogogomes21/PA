...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		char S[128];
		//------------------------ RECV #1
		printf("[RECV #1]:about to call 'recv'\n");
		ssize_t ret_recv = recv(clnt_sock, S, sizeof(S)-1, 0);
		if (ret_recv == -1) {
		...
		S[ret_recv] = '\0';
		printf("[RECV #1]received string: '%s'\n", S);
		//------------------------ SEND
		...
		//------------------------ RECV #2
		printf("[RECV #2]:about to call 'recv'\n");
		...
		S[ret_recv] = '\0';
		printf("[RECV #2]received string: '%s'\n", S);
		//------------------------

		uint16_t number = atoi(S);
		number = htons(number);
		ret_send = send(clnt_sock,&number,sizeof(number),0);
		if (ret_send == -1) {
			fprintf(stderr, "Cannot send: %s\n", strerror(errno));
			close(clnt_sock);
			continue;
